...
def extract_hpo_ids(note):
    ...
    if response.status_code == 200:
        data = response.json()
        hpo_ids = [entry["hpoId"] for entry in data.get("hmName2Id", [])]
        return hpo_ids[:5]
    else:
...
def clinphen_to_hpo(clinical_notes):
    try:
        ...
        prompt = f"""
        According to the clinical notes provided, give a list of HPO terms [In format of HP:XXXXXX] that are relevant to the patient. Do not include any additional information.
        Give space-separated HPO terms that are relevant to the patient based on the clinical notes.
        Clinical Notes:
        {clinical_notes}
        """
        ...
        if response.status_code == 200:
            ...
            # Parse HPO terms from the output
            print(llm_output)      
        ...
        st.error(f"Error extracting HPO terms: {str(e)}")
        return set()
...
def get_llama_recommendations(patient_terms, ontology):
    ...
    try:
        ...
        phenotype_names = patient_terms
        
                
        # Prepare prompt for Llama
        prompt = f"""
        Answer only what is mentioned, no additional information needed. Based on the following patient phenotypes: {', '.join(phenotype_names)}
...
with input_col:
    ...
    with st.expander("Clinical Notes to HPO Conversion", expanded=True):
        ...
        **Don't have HPO terms?**  
        ...
        """)
        if not st.session_state.hpo_generated:
            st.info("Enter clinical notes to generate HPO terms")
            ...
            st.success("HPO terms generated successfully")
            st.info("Generated HPO terms are shown below")
            st.write(st.session_state.clinical_notes)
            st.write(st.session_state.patient_terms)
            
    if st.button("Generate HPO Terms"):
        if st.session_state.clinical_notes!="":
            clinical_notes = st.session_state.clinical_notes.strip()
            with st.spinner("Analyzing clinical notes..."):
                ...
                generated_terms = extract_hpo_ids(clinical_notes)
                st.session_state.patient_terms = set(generated_terms)
                st.session_state.hpo_generated = True
                st.rerun()
    # Direct HPO Input
    with st.expander("Manual HPO Term Entry", expanded=bool(st.session_state.patient_terms)):
        prefill_value = ",".join(st.session_state.patient_terms)
                ...
                if st.session_state.patient_terms:
                    with st.spinner("Updating disease matches..."):
                        diseases = st.session_state.phrank.rank_diseases(
                            list(st.session_state.patient_terms)[-10:], 
                            top_n=10
                        )
                        if diseases:
                ...
                if st.session_state.phrank and st.session_state.patient_terms:
                    with st.spinner("Generating new recommendations..."):
                        st.session_state.llama_recommendations = get_llama_recommendations(
                            list(st.session_state.patient_terms)[-5:],
    ...
    if st.session_state.patient_terms:
        ...
        #     st.session_state.patient_terms = set(clinphen_to_hpo(st.session_state.clinical_notes))
        #     st.session_state.hpo_generated = True
        #     st.session_state.similarity_score = 0
...
if st.session_state.phrank and st.session_state.patient_terms and st.session_state.show_results:
    ...
    if float(top_score) >= score_threshold:
